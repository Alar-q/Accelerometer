...
import com.retro.androidgames.framework.hardware.impl.GLGraphics;
...
public class Vertices3 {
    ...
    public Vertices3(GLGraphics glGraphics, int maxVertices, int maxIndices,
        ...
        this.tmpBuffer = new int[maxVertices * vertexSize / 4];//Всее координаты
    ...
    public void setVertices(float[] vertices, int offset, int length) {
        ...
        for (int i = offset, j = 0; i < len; i++, j++){
            ...
            //floatToRowIntBits(9.0f) возвращает 1029102 что то такое, и это потом читается как float
        }
    ...
    public void draw(int primitiveType, int offset, int numIndeces) {
        GL10 gl = glGraphics.getGL();

        if (indices != null) {
            indices.position(offset);
            gl.glDrawElements(primitiveType, numIndeces, GL10.GL_UNSIGNED_SHORT, indices);
        } else {
            gl.glDrawArrays(primitiveType, offset, numIndeces);
        }
    }
